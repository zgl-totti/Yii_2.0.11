...
class SaleController extends BaseController{
    public function actionIndex(){
        $where=['!=','starttime',0];
        $condition=['!=','endtime',0];
        $activity=Activity::find()->where($where)->andWhere($condition);
        ...
        $factor['adposition']=5;
        $arr=['!=','top',0];
        $advertise=Advertise::find()->where($factor)->andWhere($arr)->orderBy('top desc')->asArray()->all();
    ...
    public function actionBuy(){
        ...
        $where['a.gid']=$gid;
        $info=Activity::find()->alias('a')->where($where)->joinWith('goods')->joinWith('pics')->asArray()->one();
