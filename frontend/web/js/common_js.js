...
/* $("div.hd_Shopping_list").hover(function(){
	...
	//购物车
	$("div.hd_Shopping_list").click(function(){
			$(this).addClass("hover");
			$('.dorpdown-layer').show();
			$.post(url,function(res) {
				...
				var everySum=0;
				for (var i in res){
					str+="<li class=cart"+res[i].id+">";
					str+="<div class='img'><img src='"+pub+"/Admin/Uploads/goods/"+res[i].pic+"'></div>";
					str+="<div class='content' style='width: 210px;margin: 0'><p><a href='#'>商品名称:</a>"+res[i].goodsname+"</p><p class='Price'style='float: left'> "+res[i].price+"元</p><p style='color: red;float: left'>&nbsp;x "+res[i].buynum+" </p><p style='float: right'><a href='javascript:del("+res[i].id+");' >删除</a></p></div>";
					str+="<div class='Operations'>";
					str+="</div>";
					str+="</li>";
					buynum+=parseFloat(res[i].buynum);
					everySum=res[i].buynum*parseFloat(res[i].price);
					sum+=everySum;
				}
				// buynum=buynum.toFixed(2);
				sum=sum.toFixed(2);
				$('.p_s_list').html(str);
				$('#totalnum').html(buynum);
				$('#shopping-amount').html(buynum);
				$('#totalprice').html(sum);
			},'json');
	...
	$("div.hd_Shopping_list").mouseleave(function(){
		$(this).removeClass("hover");
		$('.dorpdown-layer').hide();
	})
 //支付方式
