...
<script type="text/javascript">
    $(function(){
        $(".btn").click(function(){
            $.post("<?=\yii\helpers\Url::to(['article/add'])?>",$("#form1").serialize(),function(res){
                if(res.code==1){
                    layer.confirm(res.body,{icon:1,btn:["继续添加","算了"]},function(){
                        window.location.href="<?=\yii\helpers\Url::to(['article/add'])?>";
                    },function(){
